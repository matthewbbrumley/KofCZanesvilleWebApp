...
function CardHeader({
  ...
  return (
    <div
      ...
      className={cn(
        "group/card-header @container/card-header grid auto-rows-min items-start gap-1.5 rounded-t-[min(var(--radius-4xl),24px)] px-(--card-spacing) has-data-[slot=card-action]:grid-cols-[1fr_auto] has-data-[slot=card-description]:grid-rows-[auto_auto] [.border-b]:pb-(--card-spacing) rounded-md bg-card shadow-sm hover:shadow-md justify-end items-center w-xl mt-5 p-6 gap-(--card-spacing) flex-1 flex-col flex",
        className
      )}
      {...props} />
  );
